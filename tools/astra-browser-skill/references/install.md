# astra-browser skill — setup

## 1. Install the skill

Easiest: in Astra Browser open **Settings → General → Developer**, under "Install
the astra-browser skill" click **Install** next to your agent — Claude Code
...
agent's `skills/astra-browser`, so it stays current with each Astra Browser update.
For Pi, Install also links a companion extension at
`~/.pi/agent/extensions/astra-browser`; run `/reload` in an already-open Pi
...
ln -sfn "$PWD/tools/astra-browser-skill" ~/.pi/agent/skills/astra-browser
ln -sfn "$PWD/tools/astra-browser-skill/extensions/pi" ~/.pi/agent/extensions/astra-browser
```
...
There is nothing to switch on first. Astra Browser publishes the socket whenever it
runs, so just connect: the **consent prompt** does the rest.

How it works, and why it's safe:

- Astra Browser opens a **Unix-domain socket** (not a TCP port), so nothing on the
  network — and no other user's processes — can reach it.
- The first time a given agent connects, Astra Browser identifies the connecting process
...
- *Deny* carries a scope: **Just this time** (ask again next connection),
  **For 30 min**, or **Never ask again** — each optionally widened to *all
  agents*, which turns away every agent including ones Astra Browser has never seen.
...
Nothing needs to be running — the skill starts Astra Browser itself when no browser is
up. It launches the install it is linked to (the symlink from step 1 points
inside a specific `Astra Browser.app`), so the copy you installed from is the copy that
...
To watch it happen by hand, quit Astra Browser first, then:
...
The first request triggers the consent prompt — approve it in Astra Browser (approving
...
```bash
node ~/.claude/skills/astra-browser/scripts/runner.mjs <<'EOF'
...
```bash
node ~/.claude/skills/astra-browser/scripts/selftest.mjs
```
...
```bash
node ~/.claude/skills/astra-browser/scripts/selftest-animations.mjs
```
...
After changing any mirror code, run its fixture selftest (no Astra Browser or agents
needed): `node scripts/selftest-mirror.mjs`.

## Troubleshooting

- **CDP endpoint not found**: the skill found no socket AND could not start
  Astra Browser. Either no Astra Browser is installed where it looked (a repo checkout
  tries the canary and release bundle ids; an installed skill opens the app it
  is linked to), or `PHI_NO_LAUNCH` is set. Installing the skill from the Astra Browser
  you actually run fixes the common case; `PHI_USER_DATA_DIR` overrides which
  user-data dir is searched.
- **Launched but never published its socket**: Astra Browser started and 60s passed with
  no endpoint. It is usually sitting at a login or session-restore prompt, or
  is still starting on a cold machine — check the Astra Browser window and retry.
- **Wrong install answers (canary vs stable)**: endpoint discovery prefers
  Astra Browser Canary over stable Astra Browser when BOTH advertise a live endpoint. Dead
...
- **Console shows browser steps but not the conversation**: the session
  ...
  predates the session mirror — rebuild Astra Browser so the bundled skill has it.
- **Under Pi: console commands wait for “continue”**: the companion extension
  is not loaded. Install Pi again from Astra Browser settings, then run `/reload` in Pi.
...
- **Under Cursor: console commands answer with a "queued" notice**: that is
  ...
  while the agent is still working arrives at its next astra-browser step;
...
- **Access denied**: you denied this agent. If the denial was "For 30 min" or
  ...
  "Allow agents to control Astra Browser (CDP)" off and on, which clears every block.
...
- **Endpoint not responding / first call hangs**: the first connection waits on
  the consent prompt — approve it in Astra Browser. If Astra Browser shows no prompt at all, check
  that Astra Browser itself is responsive.
...
- **Under Codex: "network-disabled sandbox" / endpoint not responding on
  ...
  syscalls, which includes connecting to Astra Browser's unix socket — Astra Browser is fine and
...
- **"No Astra Browser app connection available"**: the CDP endpoint is up but the Mac
  ...
  infrastructure). Open a Astra Browser window and retry; if it persists, relaunch
  Astra Browser. Distinct from "unknown method" below, which means the
  framework itself is too old.
- **"PhiAgentSpace.sendMessage" unknown method**: the running Astra Browser Framework
  predates the PhiAgentSpace domain. Rebuild it:
  `autoninja -C out/PhiRelease "Astra Browser Framework.framework"` in chromium/src,
...
- **create_failed from enterContext**: no browser window is open yet —
  open one Astra Browser window first, then retry.
